...
Gemini CLI dispatch is retired in ariadnev. Do not install or invoke the old
binary for MCP discovery, research, scouting, or tool execution.

This file intentionally keeps its historical path. Existing ariadnev updates
...
3. Use `av:chrome-profile` before Chrome DevTools MCP when real profile state matters.
...
Antigravity (`agy`) remains ariadnev's third-runtime direction. It is not an
ariadnev install target or a substitute MCP bridge until its adapter and config
