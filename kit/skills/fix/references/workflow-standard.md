...
1. Activate `av:scout` skill OR launch 2-3 parallel `Explore` subagents when delegation is explicitly requested/permitted.
...
2. Activate `av:debug` skill. Use `debugger` subagent if needed.
3. Activate `av:sequential-thinking` — form hypotheses through structured reasoning.
4. Use delegated `Explore` subagents to test hypotheses only when delegation is explicitly requested/permitted.
5. If 2+ hypotheses fail → auto-activate `av:problem-solving`.
...
approaches or an architecture decision remain, activate `av:brainstorm` and
...
- Apply `av:problem-solving` skill if stuck
- Use `av:sequential-thinking` for complex logic
...
|------|------------------|
| 1 | `av:scout` OR parallel `Explore` subagents when delegation is permitted |
| 2 | `av:debug`, `av:sequential-thinking`, optional delegated debugger/Explore when permitted, (`av:problem-solving` auto), conditional `av:brainstorm` after diagnosis |
...
**Visual Assets:** Use `av:ai-multimodal` for visual assets generation, analysis and verification.
