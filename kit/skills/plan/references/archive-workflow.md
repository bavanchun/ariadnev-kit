...
### Step 2: Summarize the plans and document them with `/av:journal` skill invocation
...
- Run `av plan archive <plan-dir>` (and/or `av plan cleanup` for a retention
  sweep of stale closed plans — dry-run by default). Run `av plan --help` and
...
- If `av` is unavailable, report the skip and leave the files untouched; do not
...
- Stage and commit the changes (Use `/av:git` for commit flow)
- Commit and push the changes (Use `/av:git` for push flow)
