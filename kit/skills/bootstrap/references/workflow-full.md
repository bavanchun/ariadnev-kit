...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `av:ai-multimodal` skill generation
...
4. If no logo provided: generate with `av:ai-multimodal` skill
5. Screenshot wireframes with `av:agent-browser` -> save to `./docs/wireframes/`
...
Activate **av:plan** skill: `/av:plan --hard <requirements>`
...
Activate **av:cook** skill: `/av:cook <plan-path>` (interactive mode — review gates at each step)
