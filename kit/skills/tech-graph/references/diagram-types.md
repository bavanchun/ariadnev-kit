# Diagram Types, Layout Rules, and Shape Vocabulary

The per-type layout rules the workflow's "plan layout" step applies, the UML
coverage map, and the shape vocabulary shared by every type. Read this when the
diagram type has been classified and the layout needs its exact rules.
