...
# av:security — Security Audit

Runs a structured STRIDE + OWASP security audit on a given scope. Produces a severity-ranked findings report. With `--fix`, applies fixes iteratively using the av:autoresearch guard pattern.
...
|------|-----------|----------|
| Audit only | `/av:security <scope>` | Scan → categorize → report (one-shot) |
| Red-team discovery | `/av:security <scope> --red-team` | Iterate 4 attacker personas → STRIDE/OWASP sweep → report |
| Bounded red-team | `/av:security <scope> --red-team --iterations N` | Cap persona discovery to N iterations total |
| Audit + Fix | `/av:security <scope> --fix` | Scan → categorize → fix iteratively |
| Red-team + Fix | `/av:security <scope> --red-team --fix` | Full persona discovery → fix confirmed Critical/High |
| Bounded fix | `/av:security <scope> --fix --iterations N` | Limit fix iterations to N |
...
4. Uses `av:autoresearch` guard pattern for regression prevention
...
- Run after `av:predict` when the security persona flags concerns
- Feed Critical/High findings into `av:autoresearch --fix` for automated remediation
- Use `av:scenario` with `--focus authorization` for deeper auth flow testing
- Pair with `av:plan` to schedule Medium/Low findings as sprint tasks
