...
The vc distribution does not bundle the upstream AgentKit discovery scripts.
Use the current session's documentation and web capabilities directly; do not
claim that `detect-topic.js`, `fetch-docs.js`, or `analyze-llms-txt.js` ran.
...
Do not install global tools or clone a repository merely because an upstream
