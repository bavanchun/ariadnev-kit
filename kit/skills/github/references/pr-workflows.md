...
`av:review-pr` and commit/push mechanics to `av:git` — this reference owns
...
2. Push via `av:git cp` (or `git push -u origin <branch>` when already
   committed).
...
Activate `av:review-pr <ref>` — it owns the full review protocol
...
For a security-focused pass, additionally activate `av:security` scoped to
...
else's. Prefer `av:review-pr <ref> --merge` / `av:git merge-pr` when
