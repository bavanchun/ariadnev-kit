...
2. **Writing language**: resolve the configured language for all human-facing
   ...
   fall back to `en` and read `.ariadnev/config.yaml` (`language` /
...
- Before creating any issue: activate `av:scout` to ground it in the codebase
  (see issue-workflows reference for the mandatory dedup + history checks).
- PR review, fix loop, or merge-with-CI-watch: activate `av:review-pr`
  (`--fix`, `--reply`, `--merge` as needed) instead of reimplementing review.
- Security scan of a PR or repo: activate `av:security` and attach its
  findings as evidence.
- Commits, pushes, branch hygiene behind a PR task: activate `av:git`.
- Fixing a CI failure found via Actions: activate `av:fix` with the exact
