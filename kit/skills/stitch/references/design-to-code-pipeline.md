...
Orchestration workflows chaining Stitch design generation with ariadnev implementation skills.
...
# 4. Activate av:frontend-design to implement React components from the spec
```
...
|-------------|-------|-------------|
| React/Vue/Svelte components | `av:frontend-design` | Converts Tailwind HTML to framework components |
| Full page with style guide | `av:ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `av:ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `av:stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use av:ui-ux-pro-max for text-based design instead
