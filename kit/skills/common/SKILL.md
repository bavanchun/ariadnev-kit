...
| Journal opt-out | `--skip-journal` flag; exact skip lines `journal skipped by --skip-journal` / `journal skipped by preference`. The config branch reads `auto` from the journal skill's own config via `av:journal`'s `scripts/resolve-config.cjs`, not from `av config prefs resolve --json` — that envelope carries no journal fields | Copy the block from `av:bootstrap` or `av:cook` rather than rewording it |
