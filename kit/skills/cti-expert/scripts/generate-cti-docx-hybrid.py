...
def ensure_pandoc():
    # The original ran `subprocess.run(["command", "-v", "pandoc"], shell=True)`,
    # which does not check anything: with shell=True the list past the first item
    # becomes arguments to the shell, so it ran a bare `command` and always
    # succeeded. When it did fail it ran `apt install -y pandoc` — installing a
    # system package, unasked, from a document-generation script.
    if shutil.which("pandoc") is None:
        raise SystemExit(
            "pandoc is required to build the .docx report. Install it first "
            "(macOS: brew install pandoc, Debian/Ubuntu: sudo apt install pandoc)."
        )
