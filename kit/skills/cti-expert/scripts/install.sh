...
gh_binary_install() {
  # Default to the user's own bin dir rather than /usr/local/bin: the common
  # path then needs no root at all. A caller that wants a system-wide install
  # can still pass one as the 5th argument.
  local tool="$1" cmd="$2" repo="$3" asset_pattern="$4" install_dir="${5:-$HOME/.local/bin}"
  if has "$cmd"; then
    log_skip "$tool ($cmd)"
    return
  fi
  ...
  if curl -sL "$url" | tar -xz -C "$tmp" 2>/dev/null; then
    ...
    if [[ -n "$bin" ]]; then
      mkdir -p "$install_dir"
      # sudo only where the destination actually requires it.
      if [[ -w "$install_dir" ]]; then mv "$bin" "$install_dir/$cmd" 2>/dev/null
      else sudo mv "$bin" "$install_dir/$cmd" 2>/dev/null || mv "$bin" "$HOME/.local/bin/$cmd" 2>/dev/null
      fi
      log_ok "$tool"
    else
...
SHARETRACE_DIR="$HOME/.claude/skills/av-cti-expert/vendor/sharetrace"
