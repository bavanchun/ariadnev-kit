...
Matching is by keyword in the heading text, case- and accent-insensitive (`CHART_KEYWORDS` in `scripts/cti_docx_postprocess.py`): risk gauge under a heading containing *executive summary*; finding charts under *findings*; timeline chart under *timeline*; entity diagram under *relationship* or *entity*; visitor charts under *visitor* or *traffic*. A heading without its keyword gets no chart and no warning — the INTSUM names in `handbook/report-template.md` all match.
