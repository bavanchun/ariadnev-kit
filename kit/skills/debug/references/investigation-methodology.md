...
- Use `/av:scout` or `/av:scout ext` to find relevant files whenever context is missing or conflicting
- Use `av:repomix` only when a broad packaged snapshot materially improves the investigation; do not generate a mandatory summary document
- Use `av:docs-seeker` skill for package/plugin documentation
