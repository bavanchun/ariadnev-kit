...
Current `/output-style` discovery has not been verified by an ariadnev runtime
...
keeps an inert `.ariadnev/output-styles/` sidecar because it is not an installed
...
with a warning; a routine `av update` never overwrites it. `av kit init --force`
