...
Where `kongming` is consulted while cooking. Read when the invocation carries
`--advice`.
...
**When the workflow reaches a PR** (for example, handed off to `av:ship`): pass
`--advice` to the downstream skill so supervision persists across the handoff.
Watch and fix CI until every required check is green, then spawn `kongming` to
review the whole implementation and post its assessment plus concrete next
steps as a comment directly on the PR and the source issue (when one exists).
