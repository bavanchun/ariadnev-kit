...
description: Open the ariadnev plans dashboard in the CLI config UI. Use for plan kanban views, progress tracking, timeline checks, and quick navigation into plan files.
user-invocable: true
...
Thin launcher for the ariadnev CLI dashboard plans view.

It opens the integrated dashboard at `http://127.0.0.1:8766/plans` instead of starting the legacy standalone server.
If the dashboard is already running on another port, the launcher follows the URL reported by `av config status --json`.
...
```bash
av config start --port 8766 --no-open --no-interactive
```
...
- The live `av plan --help` surface is authoritative for dependency/status and
...
For direct dashboard or plan operations, inspect `av config --help` or
`av plan --help` at runtime. Those help surfaces own the current commands,
flags, and examples.
...
The dashboard at `/plans` is only opened when the running ariadnev CLI instance supports it, detected by either:
...
**`av` not found**
Install the ariadnev CLI and confirm `av --version` works in your shell, or set `ARIADNEV_CLI` to the local binary path.

**Dashboard did not open**
Start it manually with `av config start --port 8766`, then open `/plans` on whichever port the CLI reports.

**Need to stop a launcher-started dashboard**
Run the launcher again with `--stop`, or run `av config stop`.

**Need custom host or different port**
Run `av config start` directly with the flags you need. The `plans-kanban` launcher intentionally stays thin and opinionated.
