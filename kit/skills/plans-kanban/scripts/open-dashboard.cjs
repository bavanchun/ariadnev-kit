...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (arg === '--stop') {
      ...
      warn(`ignoring deprecated ${arg}${value ? ` ${value}` : ''}; ariadnev dashboard owns plan scope`);
    } else if (arg === '--background' || arg === '--foreground') {
      warn(`ignoring deprecated ${arg}; dashboard process is managed by av config`);
...
function akBin() {
  return process.env.ARIADNEV_CLI || 'av';
}
...
async function main() {
  ...
  if (args.stop) {
    ...
    if (result.error) {
      console.error(`[plans-kanban] av not found; install ariadnev CLI or set ARIADNEV_CLI`);
      process.exit(1);
    }
  ...
  if (!baseURL) {
    console.error('[plans-kanban] dashboard did not expose /api/plans. Upgrade ariadnev CLI or run `av config start --no-open` manually.');
    process.exit(1);
  }
