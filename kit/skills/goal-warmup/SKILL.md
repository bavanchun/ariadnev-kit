...
Composes `av:advise` and `av:plan` **by reference** — do not fork their bodies.
Does not replace `av:vibe` or `av:issue-to-plan`.
...
6. **Warmup-local review only** — do **not** run default `/av:plan red-team` apply
   ...
   global `av:plan red-team` skill files.
...
### 2. Outcome Contract (compose av:advise patterns)

Reuse `av:advise` interview patterns (one question at a time; do not fork advise
bodies). Prefer activating `/av:advise` when a full interview is needed; for an
...
Invoke `av:plan` (or equivalent steps) with the contract as hard constraint.
...
**Do not** run default `/av:plan red-team` through its apply/consistency steps.
...
- Not a generic orchestrator (`av:vibe`, `av:issue-to-plan` stay separate).
...
**Typically follows:** raw long-run goal idea, `/av:advise`  
**Typically precedes:** Codex `/goal` or Claude long-run session (user-started)  
**Related:** `/av:codex-goal` (draft goal wording), `/av:plan`, `/av:issue-to-plan`
