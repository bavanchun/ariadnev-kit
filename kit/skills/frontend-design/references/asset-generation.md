...
Complete workflow for generating design-aligned visual assets using `av:ai-multimodal` skill.
...
Use `av:ai-multimodal` skill's image generation capabilities:
...
Use `av:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `av:media-processing` skill for post-generation adjustments
