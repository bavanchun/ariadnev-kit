...
Entry point for using the `av:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `av:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `av:frontend-design` with `av:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `av:agent-browser`
Use `av:agent-browser` to capture screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `av:media-processing` Skill
