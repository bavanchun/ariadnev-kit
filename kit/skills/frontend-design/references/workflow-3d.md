...
## Prerequisites
- Activate `av:ui-ux-pro-max` skill first
- Activate `av:threejs` skill for 3D and WebGL expertise
- Have `av:ai-multimodal` skill ready for asset generation

## Initial Research
Run `av:ui-ux-pro-max` searches:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/av-ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 ${CLAUDE_PLUGIN_ROOT}/skills/av-ui-ux-pro-max/scripts/search.py "immersive 3d" --domain style
python3 ${CLAUDE_PLUGIN_ROOT}/skills/av-ui-ux-pro-max/scripts/search.py "animation" --domain ux
```
...
Use `av:ai-multimodal` skill for:
- Textures and materials
- Skyboxes and environment maps
- Particle sprites
- Video backgrounds

Use `av:media-processing` skill for:
