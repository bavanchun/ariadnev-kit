...
```bash
av journal validate <slug-or-filename-stem>
```
...
6. Browse existing entries with `av journal list` / `av journal show <slug>`, or the Journals page in desktop/dashboard.

**Optional:** Invoke the `journal-writer` subagent when emotional honesty and failure archaeology are the point of the entry; still persist through `av journal create`.
...
Explicit `/av:journal` and `av journal create` are always available and are
...
`.ariadnev/journal.yaml` and `.ariadnev/config.yaml` / `~/.ariadnev/config.yaml`
...
`zernio auth:login` already run), and a `.ariadnev/journal.yaml` with at
least one channel configured (`references/channels-config.md`).

**Workflow:**

1. Write and persist the journal via `av journal create` as above.
...
that yourself first — invoke the installed av-design/av-frontend-design
skill (image) or the installed av-hyperframes/av-remotion skill (video) —
