...
`av journal ... --social` can attach one image and/or one video to a post.
...
the installed av-design or av-frontend-design skill (image) or the installed
av-hyperframes or av-remotion skill (video) to produce a file, then pass that
...
2. `journal.video.engine` in project `.ariadnev/config.yaml` or
   `.ariadnev/journal.yaml`.
3. `journal.video.engine` in user `~/.ariadnev/config.yaml`.
4. Auto-detect: probes `~/.claude/skills/av-hyperframes/SKILL.md` — if
