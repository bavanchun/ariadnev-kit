...
|---|--------|-------|-----------|
| 1 | `<project>/.ariadnev/journal.yaml` | project, journal-specific | all `journal.*` keys, at the file's top level (no `journal:` wrapper needed) |
| 2 | `<project>/.ariadnev/config.yaml` | project, general config | the `journal:` block |
| 3 | `~/.ariadnev/config.yaml` | user, general config | the `journal:` block |
| 4 | built-in defaults | — | — |
...
looking for `.ariadnev/` or `.git/`; pass `--project-root <path>` to skip
discovery.
...
## Example: project `.ariadnev/journal.yaml`
...
## Example: user `~/.ariadnev/config.yaml`
