...
`.ariadnev/journal.yaml` (project-scoped, not committed with secrets — only
...
channels:
  - id: <string, unique>            # referenced by --channels and posted.json
    ...
    build_in_public: <bool, optional>  # tag for av-ship --social targeting

groups:
  build_in_public: [<channel-id>, ...]  # optional named group, e.g. used by av-ship --social
```
...
| `build_in_public` | no | Informational tag; `groups.build_in_public` is what `av-ship --social` actually reads. |
