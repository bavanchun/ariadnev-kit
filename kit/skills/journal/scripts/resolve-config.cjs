#!/usr/bin/env node
/**
 * Resolve the merged av-journal configuration for a project.
 *
 * Precedence (highest → lowest):
 *   1. Project `.ariadnev/journal.yaml` (`journal.*` fields at top level)
 *   2. Project `.ariadnev/config.yaml` `journal.*` block
 *   3. User `~/.ariadnev/config.yaml` `journal.*` block
 *   4. Built-in defaults
 *
...
/** Walk up from `startDir` looking for `.ariadnev/` or `.git/`; falls back to startDir. */
function findProjectRoot(startDir) {
  let dir = path.resolve(startDir);
  const { root } = path.parse(dir);
  // eslint-disable-next-line no-constant-condition
  while (true) {
    if (fs.existsSync(path.join(dir, '.ariadnev')) || fs.existsSync(path.join(dir, '.git'))) {
      return dir;
    }
...
function resolveConfig(options = {}) {
  ...
  const userConfig = readYamlFile(path.join(homeDir, '.ariadnev', 'config.yaml'));
  const projectConfig = readYamlFile(path.join(projectRoot, '.ariadnev', 'config.yaml'));
  const projectJournalYaml = readYamlFile(path.join(projectRoot, '.ariadnev', 'journal.yaml'));
