...
function makeProject() {
  const projectRoot = makeTmpDir('av-journal-generate-video-project-');
  fs.mkdirSync(path.join(projectRoot, '.ariadnev'), { recursive: true });
  return projectRoot;
}

function makeSkillsRootWithHyperframes() {
  const root = makeTmpDir('av-journal-generate-video-skills-');
  fs.mkdirSync(path.join(root, 'av-hyperframes'), { recursive: true });
  fs.writeFileSync(path.join(root, 'av-hyperframes', 'SKILL.md'), '# av-hyperframes\n');
  return root;
}
...
test('generate-video: --video <path> returns array of resolved paths', async () => {
  const fixturesDir = makeTmpDir('av-journal-generate-video-fixtures-');
  try {
...
test('generate-video: engine auto-detect falls back to remotion when hyperframes is absent', () => {
  const skillsRoot = makeTmpDir('av-journal-generate-video-skills-empty-');
  try {
    const engine = resolveEngine({ configuredEngine: 'auto', env: { MOCK_INSTALLED_SKILLS_ROOT: skillsRoot } });
  ...
  try {
    ...
    const result = await generateVideos({
      ...
      env: {
        ...
        MOCK_INSTALLED_SKILLS_ROOT: makeTmpDir('av-journal-generate-video-skills-none-'),
      },
    });
...
test('generate-video: cache hit — same key returns cached path without regeneration', async () => {
  ...
  try {
    ...
    const mockOutput = writeMockSource(fixturesDir);
    const env = { ...process.env, MOCK_MULTIX_OUTPUT: mockOutput, MOCK_INSTALLED_SKILLS_ROOT: makeTmpDir('av-journal-generate-video-skills-x-') };
...
test('generate-video: --dry-run skips generation entirely (no file written, mock untouched)', async () => {
  ...
  try {
    ...
    const result = await generateVideos({
      ...
      cwd: projectRoot,
      dryRun: true,
      env: { ...process.env, MOCK_MULTIX_OUTPUT: mockOutput, MOCK_INSTALLED_SKILLS_ROOT: makeTmpDir('av-journal-generate-video-skills-dry-') },
    });
