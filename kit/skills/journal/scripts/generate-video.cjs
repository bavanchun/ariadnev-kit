 ...
 * branch. Local-engine orchestration (via the installed av-hyperframes or
 * av-remotion skill) is the calling agent's job; the agent passes the
 ...
 * `~/.claude/skills/av-hyperframes/SKILL.md` presence — hyperframes if
...
function resolveEngine({ cliEngine, configuredEngine, env = process.env } = {}) {
  ...
  const hyperframesSkillPath = path.join(installedSkillsRoot(env), 'av-hyperframes', 'SKILL.md');
