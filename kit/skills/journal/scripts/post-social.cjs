...
async function main() {
  ...
  if (!env.ZERNIO_API_KEY) {
    console.error(
      '[post-social] ZERNIO_API_KEY is not set. Set it via .ariadnev/.env, ~/.ariadnev/.env, process.env, or run `zernio auth:login` (see references/zernio-integration.md).'
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (targetChannels.length === 0) {
    console.error('[post-social] no channels to post to — check .ariadnev/journal.yaml `channels:` and any --channels filter');
    process.exitCode = 1;
    return;
  }
