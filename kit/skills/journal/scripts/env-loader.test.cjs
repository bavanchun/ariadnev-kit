...
test('loadEnv: missing files across the whole cascade returns empty object', () => {
  const projectRoot = makeTempDir('av-journal-env-missing-');
  const homeDir = makeTempDir('av-journal-env-missing-home-');
...
test('loadEnv: cascade precedence — project .ariadnev/.env wins over user .ariadnev/.env, .claude/.env layers, but process.env still wins via resolveEnv', () => {
  const projectRoot = makeTempDir('av-journal-env-cascade-');
  const homeDir = makeTempDir('av-journal-env-cascade-home-');
  const originalHome = process.env.HOME;
  try {
    process.env.HOME = homeDir;
    ...
    fs.writeFileSync(path.join(homeDir, '.ariadnev', '.env'), 'VAR=ariadnev-user\n');

    fs.mkdirSync(path.join(projectRoot, '.ariadnev'), { recursive: true });
    fs.writeFileSync(path.join(projectRoot, '.ariadnev', '.env'), 'VAR=ariadnev-project\n');

    const fileEnv = loadEnv(projectRoot);
    assert.equal(fileEnv.VAR, 'ariadnev-project');
    assert.equal(fileEnv.ONLY_LEGACY_USER, '1');

    delete process.env.VAR;
    assert.equal(resolveEnv('VAR', projectRoot), 'ariadnev-project');
...
test('resolveEnv: falls back to defaultValue when key is absent everywhere', () => {
  const projectRoot = makeTempDir('av-journal-env-default-');
  const homeDir = makeTempDir('av-journal-env-default-home-');
  const originalHome = process.env.HOME;
  try {
    process.env.HOME = homeDir;
...
test('resolveAllEnv: merges file cascade with process.env, process.env winning', () => {
  const projectRoot = makeTempDir('av-journal-env-merge-');
  const homeDir = makeTempDir('av-journal-env-merge-home-');
  const originalHome = process.env.HOME;
  try {
    process.env.HOME = homeDir;
    fs.mkdirSync(path.join(projectRoot, '.ariadnev'), { recursive: true });
    fs.writeFileSync(path.join(projectRoot, '.ariadnev', '.env'), 'FILE_ONLY=from-file\nOVERRIDDEN=from-file\n');
