#!/usr/bin/env node
/**
 * Environment Variable Loader for the av-journal skill.
 *
 ...
 * of the env-cascade loader. Mirrors the av-seo skill's env-loader.cjs
 ...
 *   2. <cwd>/.ariadnev/.env         (project-scoped, preferred)
 *   3. ~/.ariadnev/.env             (user-scoped, preferred)
 ...
 * inside double-quoted values only (matches av-seo's env-loader.cjs).
 *
...
function readEnvFile(filePath) {
  try {
    ...
    if (process.env.DEBUG) {
      console.error(`[av-journal env-loader] could not read ${filePath}: ${error.message}`);
    }
  }
  return {};
}
 ...
 *
 * @param {string} [cwd] project root to resolve `.ariadnev/.env` /
...
function loadEnv(cwd) {
  ...
  const layers = [
    ...
    readEnvFile(path.join(homeDir, '.ariadnev', '.env')), // 3. user
    readEnvFile(path.join(projectRoot, '.ariadnev', '.env')), // 2. project
  ];
