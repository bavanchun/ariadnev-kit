...
function makeProject() {
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'av-journal-resolve-config-'));
  fs.mkdirSync(path.join(projectRoot, '.ariadnev'), { recursive: true });
  return projectRoot;
}

function makeHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'av-journal-resolve-config-home-'));
}
...
test('resolveConfig: user ~/.ariadnev/config.yaml journal.language overrides default', () => {
  const projectRoot = makeProject();
  const homeDir = makeHome();
  try {
    fs.mkdirSync(path.join(homeDir, '.ariadnev'), { recursive: true });
    fs.writeFileSync(
      path.join(homeDir, '.ariadnev', 'config.yaml'),
      'journal:\n  language: Vietnamese\n'
    );
    withHome(homeDir, () => {
      const resolved = resolveConfig({ projectRoot });
...
test('resolveConfig: project .ariadnev/journal.yaml wins over project config.yaml and user config.yaml', () => {
  const projectRoot = makeProject();
  const homeDir = makeHome();
  try {
    fs.mkdirSync(path.join(homeDir, '.ariadnev'), { recursive: true });
    fs.writeFileSync(path.join(homeDir, '.ariadnev', 'config.yaml'), 'journal:\n  language: Vietnamese\n');
    fs.writeFileSync(path.join(projectRoot, '.ariadnev', 'config.yaml'), 'journal:\n  language: French\n');
    fs.writeFileSync(path.join(projectRoot, '.ariadnev', 'journal.yaml'), 'language: Japanese\n');
...
test('resolveConfig: project .ariadnev/journal.yaml channels array-of-maps parses correctly', () => {
  const projectRoot = makeProject();
  const homeDir = makeHome();
  try {
    fs.writeFileSync(
      path.join(projectRoot, '.ariadnev', 'journal.yaml'),
      [
        'channels:',
        '  - id: x_main',
        '    platform: x',
...
test('resolveConfig: explicit journal.writing_style overrides alphabetical discovery', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(stylesDir, 'zesty.md'), '# zesty');
    fs.writeFileSync(path.join(stylesDir, 'casual.md'), '# casual');
    fs.writeFileSync(path.join(projectRoot, '.ariadnev', 'journal.yaml'), 'writing_style: zesty\n');
...
test('resolveConfig: --json CLI output is valid parseable JSON with resolved fields', () => {
  const projectRoot = makeProject();
  const homeDir = makeHome();
  fs.writeFileSync(path.join(projectRoot, '.ariadnev', 'journal.yaml'), 'language: Spanish\n');
  try {
