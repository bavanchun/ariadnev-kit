...
async function withTempPrefs(fn) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'av-show-off-prefs-'));
...
test('resolve defaults under ARIADNEV_HOME', async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'av-home-'));
  try {
    const loaded = await loadPreferences({ env: { ARIADNEV_HOME: dir } });
    assert.equal(loaded.path, path.join(dir, 'show-off', 'preferences.json'));
    assert.equal(loaded.preferences.publishing, true);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
  }
