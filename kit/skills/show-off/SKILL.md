...
ultrathink
Activate `av:frontend-design` skill to create a showcase HTML presentation for the following request:
...
The helper stores preferences at `$ARIADNEV_HOME/show-off/preferences.json`, or
`~/.ariadnev/show-off/preferences.json` when `ARIADNEV_HOME` is unset.
...
After resolving preferences, invoke `/av:project-management` **before** reading/analyzing the request content or doing any content workflow work. This skill owns plan/task lifecycle; `show-off` is a consumer.
...
- Activate `av:frontend-design` skill to create a stunning HTML file:
