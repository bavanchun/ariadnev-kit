# `av:handover` Job Spec Template

`av:handover` builds one deterministic single-job YAML spec and dispatches
it through `av:orchestrate`. This file defines the template and the field
mapping rules.

**Do not restate** the full job-spec schema, runtime matrix, or
model-routing policy — those live in
[`av:orchestrate/references/job-spec.md`](../../av-orchestrate/references/job-spec.md),
[`runtime-matrix.md`](../../av-orchestrate/references/runtime-matrix.md),
and [`model-routing.md`](../../av-orchestrate/references/model-routing.md).
...
- `agent:` — meaningful only for `runtime: internal` (per `job-spec.md`).
  ...
  `av:orchestrate/references/internal-routing.md`.
- `skill:` — meaningful only for `runtime: av-run`. Set to the target
...
Before calling `av:orchestrate`, verify the built spec by inspection:
...
`av:orchestrate` convention. The handoff artifact path is preserved as an
