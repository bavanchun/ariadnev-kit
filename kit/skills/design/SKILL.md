...
4. **Export** — Screenshot to PNG at exact dimensions via `av:agent-browser`, Chrome headless, or Playwright
...
4. **Design** — `the marketing brand skill` → `the marketing design-system skill` → randomly invoke `/av:ui-ux-pro-max` OR `/av:frontend-design`; HTML per idea × size
5. **Export** — `av:agent-browser`, Chrome headless, or Playwright screenshot at exact px (2x deviceScaleFactor)
6. **Verify** — Use Chrome MCP / `chrome-devtools-mcp`, `av:agent-browser`, or Playwright to visually inspect exported designs; fix layout/styling issues and re-export
