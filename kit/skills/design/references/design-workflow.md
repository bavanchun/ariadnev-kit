...
For HTML/CSS craft numerics (spacing, contrast, whitespace %, palette rules), defer to `av-frontend-design`. This file owns the *sequence*: brief → confirm → generate → critique → deliver.
...
- Type / spacing / contrast micro-tuning — defer numeric standards to `av-frontend-design`.
...
- HTML/CSS numeric standards (spacing, type ratio, contrast, palette rules, whitespace) → `av-frontend-design`. This file describes the sequence, not the numbers.
