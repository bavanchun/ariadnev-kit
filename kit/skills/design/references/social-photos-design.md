...
Present ideas to user via `ask_user capability` for approval before designing.

### Step 4: Design HTML Files

Activate these skills in sequence:

1. **`the marketing brand skill`** — Extract brand colors, fonts, voice from user's project
2. **`the marketing design-system skill`** — Get design tokens (spacing, typography scale, color palette)
3. **Randomly invoke ONE of:** `/av:ui-ux-pro-max` OR `/av:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Use Chrome headless, `av:agent-browser`, or Playwright/Puppeteer to capture exact-size screenshots.
...
Invoke `/av:agent-browser` with instructions to:
...
Use Chrome MCP / `chrome-devtools-mcp`, `av:agent-browser`, or Playwright to visually inspect each exported PNG:
