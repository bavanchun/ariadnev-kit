...
if ! command -v rmbg &> /dev/null; then
    ...
    echo "Install it first:  npm install -g rmbg-cli"
    # Installing a global package is a change to the user's machine, and a
    # skill script is not the place to make one unasked. The upstream version
    # ran the install itself the first time the tool was missing.
    exit 1
fi
