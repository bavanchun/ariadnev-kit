...
**Note:** HTML generation mode (`/av:preview --html ...`) produces self-contained HTML files that open directly in the browser — they do not use this server.
...
Use `/av:preview` for quick access:

```bash
/av:preview plans/my-plan/plan.md    # View markdown file
/av:preview plans/                   # Browse directory
/av:preview --stop                   # Stop server
```
