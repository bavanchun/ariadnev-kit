# `av:handoff` Artifact Schema

The handoff artifact is a Markdown document with a fixed section order. This
schema is validated by `av:handover` before dispatch (by heading name,
...
`handoff-version` lets `av:handover` reject artifacts written by a future
incompatible schema.
...
Additional H2 sections may appear between required ones. `av:handover`
...
## Validation summary (used by `av:handover`)
...
Any failure is a hard blocker — `av:handover` refuses to dispatch.
