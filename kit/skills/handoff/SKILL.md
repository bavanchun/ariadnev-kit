...
metadata:
  ...
  author: upstream
  version: "2.0.0"
  upstream: "Pinned MIT source archive: handoff@ce70edaa26247b84c2b9491a0cdb4964f65cf3a5 (rewritten for ariadnev v2 contract)"
---
...
composition belongs to [`av:handover`](../av-handover/SKILL.md).

## Boundary vs `av:watzup`

- `av:handoff` = continuation contract for a **successor agent**. Session
  reasoning, decisions, verification state, exact next actions.
- `av:watzup` = status report for a **human**, derived from branches,
...
```bash
/av:handoff
/av:handoff "continue the OAuth callback fix"
/av:handoff --output plans/handoffs/oauth-callback.md
/av:handoff --include-diff --include-status
/av:handoff --force --output plans/handoffs/oauth-callback.md
```
...
- Also scan legacy `plans/reports/handoff-*.md` so muscle memory from earlier
  ariadnev versions keeps working.

## Required Artifact Structure

Every handoff document contains these nine H2 sections in this order — the
schema is validated by `av:handover` before dispatch. See
...
**When** `/av:handoff` runs with no arguments.
...
**When** `/av:handoff --include-diff --include-status` runs.
...
**When** `/av:handoff --output plans/handoffs/oauth-callback.md` runs.
...
- No runtime dispatch, prompt assembly for another agent, or preflight — use
  [`av:handover`](../av-handover/SKILL.md).
