...
/**
 * Find an optional project-local .avignore at the git root config directory.
...
function findProjectCkignore(startDir, configDirName) {
  ...
  const candidate = path.join(gitRoot, configDirName, '.avignore');
 ...
 * @param {string} [params.options.avignorePath] - Path to .avignore file
 * @param {string} [params.options.projectCkignorePath] - Explicit project-local .avignore path
 * @param {string} [params.options.claudeDir] - Path to .claude or .opencode directory
 * @param {string} [params.options.cwd] - Working directory used to discover a project .avignore
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Resolve .avignore path
  const resolvedCkignorePath = ckignorePath || path.join(claudeDir, '.avignore');
