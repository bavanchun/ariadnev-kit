...
function resolveAriadnevHome(environment = process.env) {
  const home = homeDirectory(environment);
  const candidate = canonicalCandidate(environment.ARIADNEV_HOME || path.join(home, '.ariadnev'));
  if (!candidate) return null;
  const providerCandidates = [
    environment.ARIADNEV_CLAUDE_HOME,
...
function projectStatePaths(context, environment = process.env) {
  const root = resolveAriadnevHome(environment);
