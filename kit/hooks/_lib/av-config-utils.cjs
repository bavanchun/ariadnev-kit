/**
 * Shared utilities for ariadnev hooks
 *
...
/**
 * Load config with cascading resolution: DEFAULT → resolved ariadnev prefs
 *
 * The values come from `av-config-client`, which reads the user config and the
 * project config directly and applies the layer rule: workspace keys may come
 * from either, user-only keys from the user's file alone. Defaults stay here
 * rather than being assumed present, so a key the config does not mention falls
 * back to DEFAULT_CONFIG — which is also what an unreadable config produces.
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.AV_DEBUG) {
      console.error(`[av-config] Warning: ${validation.error}`);
    }
  }
