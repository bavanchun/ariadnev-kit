...
const { sanitizeDeep } = require('./sanitizer.cjs');

const LOG_DIR = process.env.AV_HOOK_LOG_DIR || path.join(__dirname, '..', '.logs');
...
function logHook(hookName, data) {
  try {
    ...
    // Every entry goes through redaction before it is serialized. A hook that
    // logs a failed notification would otherwise write the destination — token
    // and all — into a file nothing later reads with a filter.
    const serialized = JSON.stringify(sanitizeDeep(entry)) + '\n';
