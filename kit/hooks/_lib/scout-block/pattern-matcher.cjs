 ...
 *
 * Uses 'ignore' package for .avignore parsing and path matching.
...
// Default patterns if .avignore doesn't exist or is empty
...
function readPatternsFromFile(filePath) {
  ...
  try {
    ...
    console.error('WARN: Failed to read .avignore:', error.message);
    return [];
  }
}

/**
 * Load patterns from the shipped .avignore plus an optional project override.
 * Falls back to DEFAULT_PATTERNS if the shipped file doesn't exist or is empty.
 *
 * @param {string} ckignorePath - Path to shipped/global .avignore file
 * @param {string} [projectCkignorePath] - Optional project-local .avignore path
 ...
 *
 * @param {string[]} patterns - Array of patterns from .avignore
 ...
 *
 * @param {string[]} originalPatterns - Original patterns from .avignore
