 ...
 * av:ship and av:review-pr.
 *
 * Precedence (first non-empty wins):
 *   1. ARIADNEV_LANGUAGE
 *   2. CK_RESPONSE_LANGUAGE
 *   3. ariadnev config  locale.responseLanguage
 *   4. default "en"
 *
 * The config value arrives from `av config prefs resolve`, which merges the
...
function resolveWritingLanguage(options = {}) {
  ...
  const candidates = [
    { source: 'env:ARIADNEV_LANGUAGE', raw: env.ARIADNEV_LANGUAGE },
