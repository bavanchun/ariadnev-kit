...
const {
	...
	readSessionState,
	updateSessionState,
} = require("./av-config-utils.cjs");

function getUsageCachePath() {
	return process["env"].AV_USAGE_CACHE_PATH || path.join(os.tmpdir(), "ariadnev-usage-limits-cache.json");
}
...
function buildRulesSection({ devRulesPath, skillsVenv, plansPath, docsPath }) {
	...
	lines.push(
		`- Based on the current task, invoke these skills: "av:scout", "av:debug", "av:cook", "av:fix", "av:git", "av:test", "av:code-review", "av:docs", "av:ship", "av:review-pr"`,
	);
	lines.push(
 ...
 * @param {Object} [params.config] - ariadnev config (auto-loaded if not provided)
