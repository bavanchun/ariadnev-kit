...
try {
  ...
  const { createSessionStateContext, isHookEnabled, readSessionState, toDisplayPath } = require(require('node:path').join(AV_LIB, 'av-config-utils.cjs'));
  ...
  try {
    ...
    const sessionContext = createSessionStateContext({
      ...
      cwd: process['env'].AV_PROJECT_ROOT || payload.cwd || process.cwd(),
      requireBinding: true
    });
    let planPath = null;
    ...
    if (planPath) {
      ...
      lines.push(`Optional implementation command after user approval: /av:cook ${safeDisplayValue(planMdPath)}`);
    } else {
      // Fallback when plan path unavailable
      lines.push('Optional implementation command after user approval: /av:cook {full-absolute-path-to-plan.md}');
    }
