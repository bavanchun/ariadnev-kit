...
try {
  ...
  const {
    ...
    buildContextOutput
  } = require(require('node:path').join(AV_LIB, 'project-detector.cjs'));
...
async function main() {
  ...
  try {
    ...
    const staticEnv = {
      ...
      claudeSettingsDir: claudeConfigDir(__dirname, process.cwd())
    };
    ...
    if (envFile) {
      // Session & plan config
      writeEnv(envFile, 'AV_SESSION_ID', sessionId || '');
      writeEnv(envFile, 'AV_PLAN_NAMING_FORMAT', config.plan.namingFormat);
      writeEnv(envFile, 'AV_PLAN_DATE_FORMAT', config.plan.dateFormat);
      writeEnv(envFile, 'AV_PLAN_ISSUE_PREFIX', config.plan.issuePrefix || '');
      writeEnv(envFile, 'AV_PLAN_REPORTS_DIR', config.plan.reportsDir);

      // NEW: Resolved naming pattern for DRY file naming in agents
      // Example: "251212-1830-GH-88-{slug}" or "251212-1830-{slug}"
      // Agents use: `{agent-type}-$AV_NAME_PATTERN.md` and substitute {slug}
      ...
      writeEnv(envFile, 'AV_GIT_ROOT', staticEnv.gitRoot || '');
      // Resolve each configured path against baseDir only when it is relative.
      // An absolute override (`paths.docs` / `paths.plans` set to an absolute
      // path in a config file) must be used verbatim: path.join concatenates
      ...
      // already uses in _lib/av-config-utils.cjs.
      const resolveUnderBase = (p) => (path.isAbsolute(p) ? p : path.join(baseDir, p));
      writeEnv(envFile, 'AV_REPORTS_PATH', toDisplayPath(resolveUnderBase(reportsPath)));
      writeEnv(envFile, 'AV_DOCS_PATH', toDisplayPath(resolveUnderBase(config.paths.docs)));
      writeEnv(envFile, 'AV_PLANS_PATH', toDisplayPath(resolveUnderBase(config.paths.plans)));
      writeEnv(envFile, 'AV_PROJECT_ROOT', toDisplayPath(baseDir));

      // Project detection
      writeEnv(envFile, 'AV_PROJECT_TYPE', detections.type || '');
      writeEnv(envFile, 'AV_PACKAGE_MANAGER', detections.pm || '');
      writeEnv(envFile, 'AV_FRAMEWORK', detections.framework || '');

      // NEW: Static environment info (so other hooks don't need to recompute)
      writeEnv(envFile, 'AV_NODE_VERSION', staticEnv.nodeVersion);
      writeEnv(envFile, 'AV_OS_PLATFORM', staticEnv.osPlatform);
      writeEnv(envFile, 'AV_GIT_BRANCH', staticEnv.gitBranch || '');
      writeEnv(envFile, 'AV_USER', staticEnv.user);
      writeEnv(envFile, 'AV_LOCALE', staticEnv.locale);
      writeEnv(envFile, 'AV_TIMEZONE', staticEnv.timezone);
      writeEnv(envFile, 'AV_CLAUDE_SETTINGS_DIR', staticEnv.claudeSettingsDir);

      // Locale config
      if (config.locale?.thinkingLanguage) {
        writeEnv(envFile, 'AV_THINKING_LANGUAGE', config.locale.thinkingLanguage);
      }
      if (config.locale?.responseLanguage) {
        writeEnv(envFile, 'AV_RESPONSE_LANGUAGE', config.locale.responseLanguage);
      }

      // Plan validation config (for /av:plan validate, /av:plan --hard, /av:plan --parallel)
      const validation = config.plan?.validation || {};
      writeEnv(envFile, 'AV_VALIDATION_MODE', validation.mode || 'prompt');
      writeEnv(envFile, 'AV_VALIDATION_MIN_QUESTIONS', validation.minQuestions || 3);
      writeEnv(envFile, 'AV_VALIDATION_MAX_QUESTIONS', validation.maxQuestions || 8);
      writeEnv(envFile, 'AV_VALIDATION_FOCUS_AREAS', (validation.focusAreas || ['assumptions', 'risks', 'tradeoffs', 'architecture']).join(','));

      // Coding level config (for output style selection)
      const codingLevel = config.codingLevel ?? 5;
      writeEnv(envFile, 'AV_CODING_LEVEL', codingLevel);
      writeEnv(envFile, 'AV_CODING_LEVEL_STYLE', getCodingLevelStyleName(codingLevel));
    ...
    if (envFile && teamInfo) {
      writeEnv(envFile, 'AV_AGENT_TEAM', teamInfo.teamName);
      writeEnv(envFile, 'AV_AGENT_TEAM_MEMBERS', teamInfo.memberCount);
    }
