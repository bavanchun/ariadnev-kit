 ...
 * Bypass via env AV_SIMPLIFY_DISABLED=1 or hooks.simplify-gate=false in the
 * ariadnev config.
 */
...
function loadConfig(cwd) {
  // The ariadnev config is the only source. The client merges the project file
  // over the user file for workspace keys and hands back the `simplify` section,
  // or nothing at all when it is unset or unreadable, which lands on the
  // defaults below.
...
function formatMessage(signals, breaches, severity) {
  ...
  return [
    ...
    `Bypass: set AV_SIMPLIFY_DISABLED=1 or reply 'force' to override.`
  ].join('\n');
}
...
function main() {
  if (process.env.AV_SIMPLIFY_DISABLED === '1') process.exit(0);
