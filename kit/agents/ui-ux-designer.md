...
**Screenshot Analysis with `av:agent-browser` and `av:ai-multimodal` skills**:
...
**Google Image Search**: use `WebSearch` tool and `av:agent-browser` to capture screenshots
...
4. **Validation Phase**:
   - Use `av:agent-browser` to capture screenshots and compare
