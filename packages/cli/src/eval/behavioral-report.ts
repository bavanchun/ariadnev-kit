...
export type BehavioralVariant = "vcskill" | "agentkit";
...
function validateRun(run: BehavioralReportRun, index: number): void {
  ...
  if (!(["vcskill", "agentkit"] as string[]).includes(run.variant)) throw new Error("variant is unsupported");
...
export function buildBehavioralReport(input: {
  ...
  const cells = cellIds.map((cellId) => {
    ...
    for (const variant of ["vcskill", "agentkit"] as const) {
  ...
  for (const cell of cells) {
    if (cell.comparison.status === "matched" && (!cell.variants.vcskill || !cell.variants.agentkit)) {
