...
describe("docs bundle generator", () => {
  it("generates deterministic final sidecars and archive members from exact immutable inputs", async () => {
    ...
    execFileSync("git", ["checkout", "--quiet", "--detach", "vcskill@0.10.0"], { cwd: previousSourceTree });
    ...
    expect(bootstrap).toMatchObject({ releaseTag: "vcskill@0.10.0", productSha: previousProductSha, generatorSha: "a".repeat(40) });
