...
import { parseRegistry } from "../kit/registry.js";
...
export function runCoverage(opts: CoverageOpts = {}): CoverageResult {
  ...
  try {
    kit = loadKit(root);
    registry = parseRegistry(readFileSync(join(root, "decisions.json"), "utf8"));
  } catch (error) {
    return failed({
      skill: "(registry)",
      kind: "registry",
  ...
  for (const skill of selected) {
    ...
    const reason = result.applicable ? undefined : "no tracked claims";
