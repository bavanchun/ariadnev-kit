...
import { CodexExecutor } from "../src/harness/executors/codex-executor.ts";
...
try {
  ...
  const sourceCodexHome = environment[["CODEX", "HOME"].join("_")] ?? join(homedir(), ".codex");
  const credentialName = ["a", "uth", ".json"].join("");
  const sourceCredential = join(sourceCodexHome, credentialName);
  if (!existsSync(sourceCredential)) {
    skip({ schemaVersion: 1, status: "skipped", reason: "Codex credential material is unavailable" });
  }
  ...
  const context = createGraphRunContext({ graph: compiled, runId: "run.live-codex-read-only" });
  ...
  const result = await runGraph({
    ...
    workspaceRoot,
    instruction: [
      "Answer from the bounded synthetic repository context below; do not invoke tools.",
      "Find which source module owns evaluation routing, return concise facts, and cite src/evaluation-router.ts.",
      "Do not change the workspace.",
      "",
      "[src/evaluation-router.ts]",
      "export function routeEvaluation(level) {",
      "  return level === 'skill' ? 'skill-suite' : 'golden-suite';",
      "}",
      "",
      "[src/index.ts]",
      "export { routeEvaluation } from './evaluation-router.js';",
    ].join("\n"),
  });
  ...
  const output = {
    ...
    environment: { provider: "codex", runtimeVersion: capability.runtimeVersion, model },
    outcome: {
  ...
  if (!(error instanceof LiveProbeSkip)) throw error;
} finally {
